...
import { useUser, useOrganizations, useUpdateUser, useDeleteUser, useSetUserRegistration } from "@/lib/mdc/hooks";
...
function EditPermissionsDialog({
  ...
  const [orgRoles, setOrgRoles] = useState<UserOrganizationRole[]>(currentOrgRoles);
  ...
  const addOrgRole = () => {
    ...
    if (!exists) {
      setOrgRoles((prev) => [...prev, { organizationId: newOrgId, role: newOrgRole }]);
    }
    setNewOrgId("");
    setNewOrgRole("user");
  };
  ...
  const handleSave = async () => {
    const addApplicationRoles = appRoles.filter((r) => !currentAppRoles.includes(r));
    const removeApplicationRoles = currentAppRoles.filter((r) => !appRoles.includes(r));
    const addOrganizationRoles = orgRoles.filter(
      (r) => !currentOrgRoles.some((c) => c.organizationId === r.organizationId && c.role === r.role)
    );
    const removeOrganizationRoles = currentOrgRoles.filter(
      (c) => !orgRoles.some((r) => r.organizationId === c.organizationId && r.role === c.role)
    );
    ...
    try {
      await updateUser({
        id: userId,
        descriptor: {
          addApplicationRoles: addApplicationRoles.length ? addApplicationRoles : undefined,
          removeApplicationRoles: removeApplicationRoles.length ? removeApplicationRoles : undefined,
          addOrganizationRoles: addOrganizationRoles.length ? addOrganizationRoles : undefined,
          removeOrganizationRoles: removeOrganizationRoles.length ? removeOrganizationRoles : undefined,
        },
      });
      toast({ title: "Permissions updated", description: "User permissions have been saved successfully." });
      setOpen(false);
      onSuccess();
    } catch (err) {
      ...
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-2">
          ...
          <div className="space-y-3">
            <div>
              <Label className="text-sm font-semibold">Organization Memberships</Label>
              <p className="text-xs text-muted-foreground mt-0.5">Add or remove this user from organizations.</p>
            </div>
                ...
                {orgRoles.map((r) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium truncate">{getOrgName(r.organizationId)}</p>
                      <p className="text-xs text-muted-foreground font-mono truncate">{r.organizationId}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <OrgRoleBadge role={r.role} />
                      <button
                        type="button"
                        onClick={() => removeOrgRole(r.organizationId, r.role)}
            ...
            <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Add Membership</p>
              <div className="flex gap-2">
...
export default function UserDetailPage() {
  ...
  const { data: user, isLoading, isError, refetch, isFetching } = useUser(userId);
  const { data: organizations } = useOrganizations();
  const { mutateAsync: deleteUser, isPending: isDeleting } = useDeleteUser();
  const { mutateAsync: setRegistration, isPending: isSettingRegistration } = useSetUserRegistration();
  ...
  // Group org roles by organizationId
  const groupedOrgRoles = (() => {
    ...
    return Array.from(map.entries()).map(([orgId, roles]) => ({ orgId, roles }));
  })();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground font-mono mt-0.5">{userId}</p>
        </div>
              ...
              {user.isRegistered && (
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant="outline" size="sm" disabled={isSettingRegistration}>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        as unregistered on the MDC platform. They will lose access until re-registered.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        onClick={() =>
                          setRegistration({ id: userId, isRegistered: false })
                            .then(() => { refetch(); toast({ title: "User Unregistered", description: "The user has been marked as unregistered." }); })
                            .catch((err) => toast({ title: "Failed", description: err instanceof Error ? err.message : "An error occurred", variant: "destructive" }))
                        }
                      >
                        Unregister
                      </AlertDialogAction>
        ...
        <Card className="border-amber-300 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-700">
          <CardContent className="flex items-center justify-between gap-4 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-800 dark:text-amber-300">User not registered</p>
                <p className="text-xs text-amber-600 dark:text-amber-400">This user has not been registered on the MDC platform and cannot access resources.</p>
              </div>
            </div>
            <Button
              size="sm"
              ...
              onClick={() =>
                setRegistration({ id: userId, isRegistered: true })
                  .then(() => { refetch(); toast({ title: "User Registered", description: `${user.displayName || userId} has been registered successfully.` }); })
                  .catch((err) => toast({ title: "Failed", description: err instanceof Error ? err.message : "An error occurred", variant: "destructive" }))
              }
            >
              {isSettingRegistration ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : null}
              Register Now
            </Button>
          </CardContent>
        </Card>
            ...
            <CardContent>
              <div className="flex items-start gap-4">
                ...
                <dl className="grid grid-cols-1 gap-y-3 sm:grid-cols-2 flex-1">
                  <div>
                    <dt className="text-xs text-muted-foreground uppercase tracking-wide">Display Name</dt>
                    <dd className="mt-1 font-medium">{user.displayName || <span className="italic text-muted-foreground">—</span>}</dd>
                  </div>
                  <div>
                  ...
                  <div className="sm:col-span-2">
                    <dt className="text-xs text-muted-foreground uppercase tracking-wide">User ID</dt>
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                {(user.appRoles?.length ?? 0) > 0 && (
                  <Badge variant="secondary" className="ml-1">{user.appRoles!.length}</Badge>
                )}
              </CardTitle>
              <CardDescription>Global roles assigned on the MDC platform</CardDescription>
            </CardHeader>
            <CardContent>
              {(user.appRoles?.length ?? 0) === 0 ? (
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                {groupedOrgRoles.length > 0 && (
                  <Badge variant="secondary" className="ml-1">{groupedOrgRoles.length}</Badge>
                )}
              </CardTitle>
              <CardDescription>
                {groupedOrgRoles.length === 0
              ...
              {groupedOrgRoles.length === 0 ? (
                ...
                <div className="divide-y rounded-lg border overflow-hidden">
                  {groupedOrgRoles.map(({ orgId, roles }) => (
                    <Link key={orgId} href={`/dashboard/settings/organization/${orgId}`}>
                      <div className="flex items-center justify-between px-4 py-3 bg-background hover:bg-muted/50 cursor-pointer transition-colors gap-4">
                        <div className="min-w-0">
                          <p className="text-sm font-medium truncate">{getOrgName(orgId)}</p>
                          <p className="text-xs text-muted-foreground font-mono truncate">{orgId}</p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                          {roles.map((r) => <OrgRoleBadge key={r} role={r} />)}
